...
library(ipumsr)
library(ggplot2)
setwd("C:/Users/fredr/Dropbox/B_POSTDOC_UvA/Cities_Sample/Data_new/turkey")
...
ddi <-read_ipums_ddi("ipumsi_00231.xml")
...
table(turkey_full$OCCISCO)


turkey_full$OCCISCO_b <- ifelse(turkey_full$OCCISCO ==1|turkey_full$OCCISCO ==2,1,0)
turkey_full$OCCISCO_b <- ifelse(turkey_full$OCCISCO ==3|turkey_full$OCCISCO ==4|turkey_full$OCCISCO ==5,2,turkey_full$OCCISCO_b)
turkey_full$OCCISCO_b <- ifelse(turkey_full$OCCISCO ==6|turkey_full$OCCISCO ==7|turkey_full$OCCISCO ==8|turkey_full$OCCISCO ==9,3,turkey_full$OCCISCO_b)
table(turkey_full$OCCISCO_b)

turkey_full_OCCISCO_b <- turkey_full %>% select(YEAR,CITY,OCCISCO_b,PERWT)
turkey_full_OCCISCO_b <- turkey_full_OCCISCO_b %>% filter(OCCISCO_b !=0)

table(turkey_full_OCCISCO_b$OCCISCO_b)

OCCISCO_b_total <- turkey_full_OCCISCO_b %>% 
  group_by(YEAR,CITY) %>% 
  summarise(OCCISCO_b_total = sum(PERWT))

OCCISCO_b_TOP <- turkey_full_OCCISCO_b %>% filter (OCCISCO_b==1) %>% 
  group_by(YEAR,CITY) %>% 
  summarise(OCCISCO_b_TOP = sum(PERWT))

OCCISCO_b_MIDDLE <- turkey_full_OCCISCO_b %>% filter (OCCISCO_b==2) %>% 
  group_by(YEAR,CITY) %>% 
  summarise(OCCISCO_b_MIDDLE = sum(PERWT))

OCCISCO_b_BOTTOM <- turkey_full_OCCISCO_b %>% filter (OCCISCO_b==3) %>% 
  group_by(YEAR,CITY) %>% 
  summarise(OCCISCO_b_BOTTOM = sum(PERWT))

turkey_OCCISCO_b<- OCCISCO_b_total %>% full_join(OCCISCO_b_TOP, by = c("YEAR","CITY"))
turkey_OCCISCO_b<- turkey_OCCISCO_b %>% full_join(OCCISCO_b_MIDDLE,by = c("YEAR","CITY"))
turkey_OCCISCO_b<- turkey_OCCISCO_b %>% full_join(OCCISCO_b_BOTTOM,by = c("YEAR","CITY"))
